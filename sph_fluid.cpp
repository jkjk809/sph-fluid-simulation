...
#include <chrono>
#include <iostream>
//glsl meatball shader :)
const std::string fluidShader = R"(
    ...
    void main() 
    {
        ...
        float edgeMask = 1.0 - smoothstep(0.0, 0.15, depth);
        finalColor += vec3(0.8, 0.95, 1.0) * edgeMask * 0.4;
...
const float maxVelocity    = 800.0f;
...
void ComputeForces() 
{
    ...
    for (int i = 0; i < totalParticles; i++) 
    {
        ...
        sf::Vector2f gravity(0.0f, 800.0f * particles[particleIndex].density);
        ...
        if (mouse.active) 
        {
            ...
            //checks mouse within 80 pixels of particle
            if (mouseDistSq < 6400.0f) 
            {
                //linear ramp for mouse force
...
int main() 
{
    ...
    for (int y = 0; y < texSize; ++y) 
    {
        for (int x = 0; x < texSize; ++x) 
        {
            ...
            if (distSq < radiusSq) 
            {
                ...
                float a = std::exp(-2.75f * distSq / radiusSq);
    ...
    //compile shader
    sf::Shader shader;
    if (!shader.loadFromMemory(fluidShader, sf::Shader::Fragment)) 
    {
        std::cerr << "Failed to load shader!" << std::endl;
        return -1;
    }
    shader.setUniform("texture", sf::Shader::CurrentTexture);
    shader.setUniform("threshold", 0.05f);
    shader.setUniform("resolution", sf::Vector2f((float)WIN_W, (float)WIN_H));
    //additive blending
    ...
    while (window.isOpen()) 
    {
        ...
        while (window.pollEvent(ev)) 
        {
            ...
            else if (ev.key.code == sf::Keyboard::Space) useShader = !useShader;
        }
        ...
        if (benchmarkFrameCount >= 100) 
        {
            float avgTime = totalPhysicsTime / (float)benchmarkFrameCount;
            std::cout << "Avg Physics Time: " << avgTime << " ms" << std::endl;
            totalPhysicsTime = 0.0f;
            benchmarkFrameCount = 0;
        }

        if (acc > timestep) acc = 0.0f;
        ...
        if (useShader) 
        {
            ...
            //tune the alpha to decide contribution of alpha for cutoff
            particleSprite.setColor(sf::Color(255, 255, 255, 15)); 
            
            //stamp blob sprite over canvas for every particle
